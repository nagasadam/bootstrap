...
var payercomp_data = symphony_data.map(function(d) {
    TC_NAME: d.value.TC_NAME,
    SUB_TC_NAME: d.value.SUB_TC_NAME,
    SGMNT: d.value.SGMNT
    RX_FILL_YYYYQTR: d.value.RX_FILL_YYYYQTR,
    MNFCTRR: d.value.MNFCTRR,
    DRUG_NAME: d.value.DRUG_NAME,
    PYR_NAME: d.value.PYR_NAME,
    PYR_SLS_AMT: d.value.PYR_SLS_AMT,
    PYR_PAT_CNT: d.value.PYR_PAT_CNT,
    PYR_TRX: d.value.PYR_TRX,
    PYR_RBT_AMT: d.value.PYR_RBT_AMT
  }*/
...
unique_MNFCTRR = unique_MNFCTRR.concat(uniques(symphony_data.map(function(d) {
  return d.MNFCTRR;
})));;

//unique_MNFCTRR.sort();

unique_DRUG_NAME = unique_DRUG_NAME.concat(uniques(symphony_data.map(function(d) {
  return d.DRUG_NAME;
})));;
...
var TC_select = d3.select("#dropDown_TC").append("select").attr("width", "20%").on("change", function() {
  ...
  if (TC_NAME != "ALL") {
   ...
   unique_MNFCTRR = unique_DRUG_NAME.concat(uniques(filtered_payercomp_data_client.map(function(d) {
      return d.MNFCTRR;
    })));   
  };
...
var MNFCTRR_select = d3.select("#dropDown_MNFCTRR").append("select").attr("width", "20%").on("change", function() {
  ...
  unique_DRUG_NAME = ["ALL"].concat(uniques(filtered_payercomp_data_client.map(function(d) {
      return d.DRUG_NAME;
    })));
